// lib/src/pages/settings/categories_persons_page.dart
// صفحهٔ مدیریت دسته‌بندی‌های اشخاص — طراحی شبیه وردپرس (دو ستون)
// - کلاس اصلی: CategoriesPersonsPage (نام یکتا، سازگار با import در app.dart)
// - استفاده از AppDatabase.getPersonCategories/savePersonCategory/deletePersonCategory
// - ردیف‌ها dense تا دسته‌های زیادی قابل مشاهده باشند.
...
class CategoriesPersonsPage extends StatefulWidget {
  const CategoriesPersonsPage({super.key});

  @override
  State<CategoriesPersonsPage> createState() => _CategoriesPersonsPageState();
}
...
class _CategoriesPersonsPageState extends State<CategoriesPersonsPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await AppDatabase.getPersonCategories();
      if (!mounted) return;
      setState(() => _cats = List<Map<String, dynamic>>.from(list));
    } catch (e) {
      if (mounted) {
        NotificationService.showToast(
            context, 'بارگذاری دسته‌ها ناموفق بود: $e',
  ...
  Future<void> _save() async {
    ...
    if (!(_formKey.currentState?.validate() ?? false)) {
      NotificationService.showToast(context, 'فرم نامعتبر است',
    ...
    setState(() => _loading = true);
    try {
      await AppDatabase.savePersonCategory(item);
      _nameCtrl.clear();
      _slugCtrl.clear();
      _descCtrl.clear();
      _parentId = 0;
  ...
  Future<void> _delete(int id) async {
    ...
    if (ok == true) {
      setState(() => _loading = true);
      try {
        await AppDatabase.deletePersonCategory(id);
        await _load();
  ...
  List<Widget> _renderTreeNodes(List<_CategoryNode> nodes, [int depth = 0]) {
    ...
    for (final n in nodes) {
      ...
      if (!included && _searchQ.isNotEmpty) continue;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('دسته‌بندی‌های اشخاص')),
      body: _loading
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                const SizedBox(height: 8),
                                const Text(
                                  'توضیح: ساختار درختی مشابه وردپرس است؛ هنگام افزودن/ویرایش والد مناسب را انتخاب کنید.',
                                  style: TextStyle(
